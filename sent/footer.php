<footer>
        <div class="footer-container">
          <div class="container-marca">
            <img src="../img/logosinfondo.png" class="ml-5 footer-logo-img">
            <h2 class="user-select-none ml-5 footer-text"><i class="far fa-copyright mr-2"></i>BeFit</h2>
          </div>
          <div class="mt-3 center-footer-container">
            <dl class="row w-75 mx-auto">
              <dt class="col-sm-3">BeFit</dt>
              <dd class="col-sm-9">Copyright © 2020. Todos los derechos reservados.</dd>
            
              <dt class="col-sm-3">¿Encontraste un error?</dt>
              <dd class="col-sm-9 mb-5">
                <p>Repórtalo. <a href="contactanos">Envíanos</a> un mensaje.</p>
              </dd>
            
              <dt class="col-sm-3">Comparte la página</dt>
              <dd class="col-sm-9">Hazle conocer sobre la página a tus amigos. Dales una mano.</dd>
            
              <dt class="col-sm-3 text-truncate">Te ayudamos</dt>
              <dd class="col-sm-9">En esta página web encontrarás sobre ejercicios, información y disciplina alimentaria para medrar tu calidad de vida.</dd>
              
              <dt class="col-sm-3">        <script type='text/javascript' src='https://www.freevisitorcounters.com/auth.php?id=45feb3df2e8634d6e8862e5730fdfff85c1951dd'></script>
        <script type="text/javascript" src="https://www.freevisitorcounters.com/en/home/counter/762197/t/7"></script></dt>
            </dl>
          </div>
          <div class="container-needhelp">
            <div>
              <h3 class="user-select-none footer-texthelp">¿Estás perdido?</h3>
              <h3 class="user-select-none footer-texthelp">¿Necesitás ayuda?</h3>
              <h4><a href="contactanos" class="footer-texthelp">Contáctanos</a></h4>
            </div>
            <img src="../img/needhelp.png" class="needhelp">
          </div>
        </div>
      </footer>